...
fn main() {
    ...
    let outcome = (|| { let loaded = load_configuration(config)?; let source = PostgresDataSource::connect(&loaded.postgres)?; let mut runtime = VkgRuntime::new(loaded.spec, source)?; runtime.query(&query) })();
