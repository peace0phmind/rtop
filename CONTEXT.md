...
**property path 一次关系**：给定一个 property path 子表达式、活动图和图作用域后，该子表达式一次匹配产生的去重 `(起点, 终点, 图)` RDF term 关系；量词闭包在此关系上计算，而非在 BGP 展开产生的中间解映射上计算。
_Avoid_: sequence BGP, 路径实例计数
...
**交付模块覆盖率**：对每个已交付 Rust 源模块的插桩执行覆盖度；它同时覆盖替换内核及配置、数据源、CLI、HTTP 等交付路径，并以逐文件门槛防止由高覆盖核心模块掩盖未测适配器。
_Avoid_: 仅核心层总覆盖率, 测试数量

**功能漂移**：固定 Ontop 基线、夹具、容器、依赖版本或规范化规则改变后，闭合矩阵原有结论不再自动成立的状态。
_Avoid_: 可忽略的测试更新, 同一功能

**证据保鲜**：在基线、夹具、依赖、实现或规范化器变更时，重新执行差分用例、覆盖率采集和矩阵校验的过程。
_Avoid_: 只更新 hash, 只重跑 rtop

**差异裁决**：Ontop-vs-rtop 差分出现后，将其归类为 rtop 缺陷、Ontop 已知限制、获准的范围外行为或环境不确定性，并附有可复核依据的过程。
_Avoid_: 人工忽略, 预期不同

**独立差分 harness**：分别启动 Ontop 与 rtop、分别收集并规范化结果、最后才比较的测试执行器；它不得复用被测实现的解析、序列化或预期生成逻辑。
_Avoid_: rtop 自证, 共享预期生成器

**可判定功能原子**：在固定夹具中能独立给出通过、失败、ignore 或环境不确定性裁决的最小三层行为单元。
_Avoid_: 过大的测试类, 只有代码分支的条目

**失败指纹**：差分失败所需的最小可重放输入、基线版本、容器 digest、规范化输出和资源观察的集合。
_Avoid_: 仅 CI 日志链接, 无法复现的失败

**重放描述**：差分证据中供机器校验的结构化执行身份，由 runner、差分 case 和稳定参数组成；临时目录、PID 与其他运行时路径不属于重放描述。
_Avoid_: 原始 shell 命令全文, 临时工作目录
